...
describe("Roadmap Controller Integration Tests", () => {
  ...
  it("should return only public roadmaps via GET /api/roadmaps/public", async () => {
    ...
    await roadmapRepo.save({
      ...
      status: "PUBLISHED",
      userId: userId,
      data: { version: "1.0", viewport: { x: 0, y: 0, zoom: 1 }, nodes: [], edges: [] },
    });
    ...
    const publicRoadmap = roadmaps.find((r: any) => r.title === "Public Roadmap");
    const privateRoadmap = roadmaps.find((r: any) => r.title === "Private Roadmap");
